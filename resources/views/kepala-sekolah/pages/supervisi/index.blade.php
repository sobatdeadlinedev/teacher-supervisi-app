...
@section('content')
                            ...
                            <tbody class="text-gray-600 fw-semibold">
                                @foreach ($supervisions as $supervision)
                                    <tr data-status="{{ $supervision->status }}">
                                        ...
                                        <td class="text-end">
                                            @if ($supervision->status == 'scheduled')
                                                <a href="{{ route('kepala-sekolah.supervisi.assess', $supervision->id) }}"
                                                    class="btn btn-sm btn-primary">
                                                    <i class="ki-outline ki-notepad-edit fs-4"></i>
                                                    Nilai
                                                </a>
                                                <button type="button" class="btn btn-sm btn-light-danger"
                                                    onclick="cancelSupervisi({{ $supervision->id }})">
                                                    <i class="ki-outline ki-cross-circle fs-4"></i>
                                                    Batalkan
                                                </button>
                                            @elseif($supervision->status == 'completed')
                                                <a href="{{ route('kepala-sekolah.supervisi.show', $supervision->id) }}"
    ...
    <script>
        ...
        // Cancel supervisi
        function cancelSupervisi(id) {
            if (confirm('Yakin ingin membatalkan supervisi ini?')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = `/kepala-sekolah/supervisi/${id}/cancel`;
